    ...
    try {
        ...
        if (command === "/큐 돌리기") {
            if (player.state === "PLAYING") {
                return { ok: true, message: "이미 게임 중입니다. 라인전을 진행하며 강해져 보세요." };
            }
            ...
            const level = 1;
            const exp = 0;
            const nextExp = 100; // 지금은 고정, 나중에 경험치 커브 함수로 교체

            const skillSlot = startSkill ? getSkillSlotLabel(startSkill.id) : null;
            const skillText = startSkill
                ? `${skillSlot}(${startSkill.name}) Lv1`
                : "없음";
        ...
        if (command === "/서렌") {
            if (player.state !== "PLAYING") {
                return { ok: true, message: "현재 게임 중이 아닙니다." };
            }
            ...
            return { ok: true, message: `게임이 종료되었습니다. 다시 큐를 돌려 게임을 시작할 수 있습니다. / 현재 Lp=${updated.lp}, gold=${updated.gold}` };
        }

        return { ok: true, message: `미구현 커맨드: ${command}` };
    } catch (e) {
        if (e.code === "COOLDOWN") {
